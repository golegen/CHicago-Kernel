...
// Last edited on April 19 of 2019, at 11:08 BRT
...
Void DispIncrementProgessBar(Void) {
	if (DispProgressBar == 0) {																															// This first one is different
		DispFillRectangle(DispBackBuffer->width / 2 - 100 + (DispProgressBar * 2) + 3, DispBackBuffer->height - 24, 1, 9, 0x0000A8);
		DispFillRectangle(DispBackBuffer->width / 2 - 100 + (DispProgressBar * 2) + 4, DispBackBuffer->height - 25, 1, 11, 0x0000A8);
		DispFillRectangle(DispBackBuffer->width / 2 - 100 + (DispProgressBar * 2) + 5, DispBackBuffer->height - 26, 1, 13, 0x0000A8);
		DispFillRectangle(DispBackBuffer->width / 2 - 100 + (DispProgressBar * 2) + 6, DispBackBuffer->height - 27, 11, 15, 0x0000A8);
		DispProgressBar += 10;
	} else if (DispProgressBar < 90) {
		DispFillRectangle(DispBackBuffer->width / 2 - 100 + (DispProgressBar * 2), DispBackBuffer->height - 27, 17, 15, 0x0000A8);						// The others are... normal
		DispProgressBar += 10;
	} else if (DispProgressBar < 100) {
		DispFillRectangle(DispBackBuffer->width / 2 - 100 + (DispProgressBar * 2), DispBackBuffer->height - 27, 15, 15, 0x0000A8);						// The last one is also different
		DispFillRectangle(DispBackBuffer->width / 2 - 100 + (DispProgressBar * 2) + 15, DispBackBuffer->height - 26, 1, 13, 0x0000A8);
		DispFillRectangle(DispBackBuffer->width / 2 - 100 + (DispProgressBar * 2) + 16, DispBackBuffer->height - 25, 1, 11, 0x0000A8);
		DispFillRectangle(DispBackBuffer->width / 2 - 100 + (DispProgressBar * 2) + 17, DispBackBuffer->height - 24, 1, 9, 0x0000A8);
		DispProgressBar += 10;
	}
...
Void DispDrawProgessBar(Void) {
	DispDrawRoundedRectangle(DispBackBuffer->width / 2 - 100, DispBackBuffer->height - 30, 201, 21, 7, 0xFFFFFF);										// Draw the progress bar border
	DispWriteString((DispBackBuffer->width / 2 - 56) / 8, 1, 0x000000, 0xFFFFFF, L"Starting up...");													// And the "Starting..." text
