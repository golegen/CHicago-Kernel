...
// Last edited on January 17 of 2019, at 21:41 BRT
...
static Void PCIHandleDevice(UInt16 bus, UInt8 slot, UInt8 func, UInt16 vendor, UInt16 device, UInt8 class, UInt8 subclass) {
	if ((vendor == PCI_VENDOR_INTEL) && (device == PCI_DEVICE_E1000)) {																											// E1000?
		E1000Init(bus, slot, func);																																				// Yes :)
	} else if ((class == PCI_CLASS_MASS) && (subclass == PCI_SUBCLASS_SATA)) {																									// AHCI Controller?
		AHCIInit(bus, slot, func);																																				// Yes :)
	}
}
	...
	for (UInt16 i = 0; i < 256; i++) {																																			// Brute force scan
		for (UInt8 j = 0; j < 32; j++) {
			...
			if (vendor == 0xFFFF) {
				...
				PCIHandleDevice(i, j, 0, vendor, PCIReadWord(i, j, 0, PCI_DEVICE_ID), PCIReadByte(i, j, 0, PCI_CLASS), PCIReadByte(i, j, 0, PCI_SUBCLASS));						// Ok, let's handle it (if we need)!
			}
			...
			if ((PCIReadByte(i, j, 0, PCI_HEADER_TYPE) & 0x80) == 0x80) {																										// Multi-function device?
				for (UInt8 k = 1; k < 8; k++) {																																	// Yes, scan the functions!
					...
					if (vendor != 0xFFFF) {																																		// This device exists?
						PCIHandleDevice(i, j, k, vendor, PCIReadWord(i, j, k, PCI_DEVICE_ID), PCIReadByte(i, j, k, PCI_CLASS), PCIReadByte(i, j, k, PCI_SUBCLASS));				// Yes, handle it :)
					}
				}
			}
		}
